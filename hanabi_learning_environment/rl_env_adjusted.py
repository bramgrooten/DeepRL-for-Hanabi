...
Some small adjustments made by drl_for_hanabi Grooten."""
...
def make(environment_name="Hanabi-Full", num_players=2, obs_type="CARD_KNOWLEDGE", config=None, pyhanabi_path=None):
  """Make an environment.

  Args:
    environment_name: str, Name of the environment to instantiate.
    num_players: int, Number of players in this game.
    obs_type: str, the observation type (CARD_KNOWLEDGE, MINIMAL, or CHEAT) (added by drl_for_hanabi)
    config: dict, configuration dictionary if you want to set it yourself (added by drl_for_hanabi)
  ...
  if config is None:
      ...
      elif environment_name == "Hanabi-Cheat":
          return HanabiEnv(
              config={
                  "colors":
                      5,
      ...
      elif environment_name == "Hanabi-Cheat-2colors":
          return HanabiEnv(
              config={
                  "colors":
                      2,
      ...
      if obs_type == "MINIMAL":
          config["observation_type"] = pyhanabi.AgentObservationType.MINIMAL.value
          return HanabiEnv(config=config)
      elif obs_type == "CHEAT":
          config["observation_type"] = pyhanabi.AgentObservationType.SEER.value
          return HanabiEnv(config=config)
      else:
          config["observation_type"] = pyhanabi.AgentObservationType.CARD_KNOWLEDGE.value
          return HanabiEnv(config=config)
