...
"""
This file implements the Vanilla Policy Gradient algorithm
...
ADV_TYPE = 'basic'   # options: gae, basic, no
LAM = 0.95
...
DATA_FOLDER = f"data/vpg/{datetime.now().strftime('%Y-%m-%d')}/"
SAVE_PARAMS_EVERY = 0   # set to e.g. 1000 if you want to save the policy after every 1000 epochs. 0 means: don't save


class VPG_Agent:
    """
    class for the Vanilla Policy Gradient algorithm
    """
    def __init__(self, epochs=50, runtime=0.0, show_epoch_nr=10, print_training=True,
                 lr_pi=1e-3, hidden_layers_pi=None, activation_pi=nn.Tanh, train_pi_iters=1,
                 lr_vf=1e-3, hidden_layers_vf=None, activation_vf=nn.Tanh, train_vf_iters=5,
                 gamma=1.0, batch_size=1000, entropy_coeff=1e-3,
    ...
    def compute_loss_pi(self, obs, act, advantage, update_iter=0):
        ...
        return -(logp * advantage).mean() - self.entropy_coeff * mean_entropy, mean_entropy, avg_probs
    ...
    def update_step(self, obs, act, advantage, v_target):
        ...
        Performs train_pi_iters number of updates to the policy network (default is 1)
        ...
        Returns:
            ...
            avg_value: float (or None), the average output of the value network
        """
        # compute data needed for policy updates
        avg_value, mean_entropy, avg_probs = None, None, None
        if self.renormalize_adv:
            advantage = (advantage - advantage.mean()) / (advantage.std() + 1e-8)

        # update the policy network
        for iteration in range(self.train_pi_iters):
            self.optimizer_pi.zero_grad()
            loss_pi, entropy, probs = self.compute_loss_pi(obs, act, advantage, iteration)
            loss_pi.backward()
            self.optimizer_pi.step()
            if iteration == 0:
                mean_entropy = entropy  # just for logging
                avg_probs = probs
        ...
        return loss_pi, loss_vf, mean_entropy, avg_probs, avg_value
    ...
    def train_one_epoch(self):
        ...
        loss_pi, loss_vf, mean_entropy, avg_probs, avg_value = self.update_step(obs, act, advantage, v_target)
        ...
        return loss_pi, loss_vf, batch_rets, batch_fireworks, batch_scores, batch_lens, batch_acts, batch_lives, \
               num_eps, len(batch_obs), total_illegal, mean_entropy, avg_probs, avg_value, avg_target
    ...
    def train_n_hours(self):
        ...
        while datetime.now() < end_time:
            ...
            num_eps, batch_sz, num_ill, mean_entropy, avg_probs, avg_value, avg_target = self.train_one_epoch()
            ...
            if self.current_epoch % self.show_epoch_nr == 0:
                self.train_data.append([self.current_epoch, np.mean(avg_loss_pi), np.mean(avg_loss_vf),
                                        ...
                                        avg_probs, avg_value, avg_target])
