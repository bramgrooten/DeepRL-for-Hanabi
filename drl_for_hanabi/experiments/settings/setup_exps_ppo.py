...
adv_type = [ 'gae' ]  # options: no, basic, gae
lam = [ 0.95 ]

# weight_decay = [ 0, 1e-3, 1e-2 ]
# drop_rate = [ 0 , 0.2, 0.5]
obs_vec_type = [ 136 ]  # options: 37, 62, 136, 186
num_actions = [ 11 ]
life_tokens = [ 3 ]
hint_tokens = [ 8 ]
...
for exp in product(rew_success_play,                                                               # 0
                   ...
                   clip_param, gamma, batch_size, entropy_coeff,                                   # 17, 18, 19, 20
                   renormalize_adv, adv_type, lam,                                                 # 21, 22, 23
...
df_experiments = pd.DataFrame(experiments,
                              columns=['num_exp', 'success play', 'illegal move', 'out of lives', 'lost one life',
                                       ...
                                       'clip_param', 'gamma', 'batch_size', 'entropy_coeff',
                                       'renormalize_adv', 'adv_type', 'lambda',
                                       # 'weight_decay', 'drop_rate',
                                       'obs_vec_type', 'num_actions', 'life_tokens', 'hint_tokens',
                                       'random_seed'])
                                       # 'epsilon_start', 'epsilon_decay_type', 'epsilon_low_at_epoch', 'epsilon_low'])
